...
> **Code correspondence — stated once here and not repeated.** The repository stores the *opposite* order. `HatLayer('se3')` and `vee_se3` in `core/lie_alg_util.py` use $[v\,;\,\omega]$ (their comment reads `v = [t, omega]^T`), i.e. `x[..., 0:3]` $=v$ and `x[..., 3:6]` $=\omega$; `experiment/pc_se3_congruence/se3_utils.py` builds $\mathrm{Ad}_T$ to match, so its adjoint reads $\bigl[\begin{smallmatrix} R & \hat pR\\ 0 & R\end{smallmatrix}\bigr]$ with the coupling block in the upper right. The two orders are conjugate by the block-swap permutation $\Pi=\bigl[\begin{smallmatrix}0&I\\ I&0\end{smallmatrix}\bigr]$:
> $$\mathrm{Ad}^{[\omega;v]}_T=\Pi\,\mathrm{Ad}^{[v;\omega]}_T\,\Pi ,$$
> and $Q$ is numerically the same matrix in both orders (it *is* the swap), so $Y=QZ$ in the code needs no adjustment. Every number reported here — relative errors, ranks, dimensions, signatures, eigenvalues — is invariant under $\Pi$-conjugation, so the ordering difference never affects a result. The prose and all block matrices below use $[\omega\,;\,v]$; only literal code excerpts follow the storage order.
